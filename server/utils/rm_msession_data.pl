...
if ( (not defined $sel_machine_id) && (not defined $sel_msession_id) ) {
    print "No machine_id or msession_id selected.\n";
    pod2usage(1);
}
...
$search_attrs = { 'alias' => 'msession_id', };
my $rs_msession = $schema->resultset('msession')->search( $search_cond, $search_attrs );
...
unless ( $found ) {
    print "No msessions found for ";
    print "sel_msession_id=$sel_msession_id" if defined $sel_msession_id;
    if ( defined $sel_machine_id ) {
        print ", " if defined $sel_msession_id;
...
my $all_confs = [
    [ 'ttest', { 'trun_id' => { 'msjobp_cmd_id' => { 'msjobp_id' => { 'msjob_id' => 'msession_id' }, }, }, }, ],
    [ 'trun', { 'msjobp_cmd_id' => { 'msjobp_id' => { 'msjob_id' => 'msession_id' }, }, }, ],
    [ 'msjobp_cmd', { 'msjobp_id' => { 'msjob_id' => 'msession_id' }, }, ],
    [ 'msjobp', { 'msjob_id' => 'msession_id' }, ],
    [ 'msjob', 'msession_id', ],
    [ 'mslog', 'msession_id', ],
    [ 'msession', undef ],
];
